...
describe('ShadowQueue', () => {
  ...
  describe('staleness validation (2026-07-06 stale-read incident fix)', () => {
    ...
    it('P2-1: cwd dep 让 stat 按会话 cwd 解析相对 target（sidecar 多会话形态）', async () => {
      // process.cwd()（仓库根）下不存在 src/a.ts——stat 若错走进程 cwd 则
      // mtime 缺失 → checkHit fail-closed 全 miss → 长会话通道假 0%
      const { mkdirSync } = await import('node:fs')
      mkdirSync(join(dir, 'src'), { recursive: true })
      writeFileSync(join(dir, 'src', 'a.ts'), 'export const a = 1\n')
      const queue = new ShadowQueue({ execute: async () => 'content', cwd: dir })
      queue.enqueue({ tool: 'read_file', probability: 0.8, likelyTarget: 'src/a.ts' })
      await waitForEnqueue(queue)

      assert.equal(queue.checkHit('read_file', 'src/a.ts'), 'content',
        'stat 必须按会话 cwd 解析相对 target，与匹配键同基准')
    })
  })
})
