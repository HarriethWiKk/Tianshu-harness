...
describe('decideContinuation', () => {
  ...
  it('写工同时命中停滞判据时，skipReason 仍报架构边界——排查不该被引向模型速度', () => {
    const decision = decideContinuation(input({
      isWrite: true,
      result: result({ failureReason: 'timeout' }),
      productivity: { toolCalls: 0 },
    }))
    assert.equal(decision.proceed, false)
    if (!decision.proceed) {
      assert.match(decision.skipReason, /hands-session/, `写工的真实原因是这层不该续：${decision.skipReason}`)
      assert.doesNotMatch(decision.skipReason, /停滞/)
    }
  })
