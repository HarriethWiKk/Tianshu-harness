...
export interface ShadowQueueDeps {
  ...
  /**
   * stat 基准目录（P2-1）：相对 target 的 mtime/size 校验按此解析。缺省
   * process.cwd()——TUI/benchmark 子进程 cwd == 会话 cwd 无碍；桌面 sidecar
   * 单进程多会话 process.cwd() ≠ 会话 cwd，必须由构造方传会话 cwd，否则
   * stat 打错基准 → mtime 缺失 → checkHit fail-closed 全 miss → 长会话通道假 0%。
   */
  cwd?: string
  /**
...
function emptySourceStats(): ShadowQueueSourceStats {
  return {
    ...
    'import-graph': { enqueued: 0, hits: 0 },
  }
}
...
export class ShadowQueue {
  ...
  enqueue(prediction: ToolPrediction): void {
    ...
    void Promise.all([
      ...
      statAsync(resolve(this.cwd, target)).catch(() => null),
  ...
  checkHit(tool: string, target: string): string | undefined {
    ...
    try {
      live = statSync(resolve(this.cwd, target))
    } catch {
  ...
  statsBySource(): ShadowQueueSourceStats {
    return {
      ...
      'import-graph': { ...this.sourceStats['import-graph'] },
    }
  }
