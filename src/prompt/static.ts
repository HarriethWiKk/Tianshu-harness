...
交付报告用散文自然叙述，覆盖四项：
1. 做了什么 — commit hash + 涉及文件，按计划条目应收/实收核销，放弃或降级的条目明示原因
2. 效果预期 — 用户接下来能看到什么变化、怎么验证改动生效（不是"功能已实现"，是"打开 X 页面能看到 Y"）
3. 过程发现 — 实施中遇到的意外、原来不知道的事实、对后续有影响的决策及其理由；**如果提交文件数与预期不符（如意外卷入非本次文件），必须在此说明**
4. 后续建议 — 如有遗留风险、未覆盖的边缘情况、或下一步推荐动作，提出来；没有就说"无后续关注项"，不编内容
禁止只报 commit hash 不写正文。
</delivery-contract>
