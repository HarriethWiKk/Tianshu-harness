...
describe('connect draft · engine wiring', () => {
  ...
  // PR#38 审查阻断 5：对已配置 preset 重新接入，密钥步提交新 key 后 Esc 取消——
  // 取消路径不得用正式名覆盖在用凭证；草稿密钥只能进草稿专用引用。
  it('Esc after re-keying an existing preset keeps the in-use secret intact', () => {
    writeSecret('deepseek', 'sk-key-A')
    const { app } = makeApp()
    const a = internals(app)
    app.startConnect()
    a.connectFlow!.submitChoice('deepseek')
    // 若 preset 有 billing 步，选第一项继续前进；密钥步直喂新 key。
    for (let i = 0; i < 3; i++) {
      const v = a.connectFlow!.view()
      if (v.kind === 'multi-choice') a.connectFlow!.submitChoice((v as unknown as { options: { id: string }[] }).options[0]!.id)
      else break
    }
    a.connectFlow!.submitInput('sk-key-B')
    a.cancelConnect()
    assert.equal(readSecret('deepseek'), 'sk-key-A', 'Esc 取消不得覆盖在用凭证')
    assert.equal(readSecret('draft-deepseek'), 'sk-key-B', '草稿密钥存草稿专用引用名')
    const draft = readConnectDraft(home)
    assert.equal(draft?.collected.keyRef, 'draft-deepseek', '草稿引用指向草稿专用名')
  })
